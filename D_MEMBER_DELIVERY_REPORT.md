...
- `PtzHttpClient.cc`：libcurl 访问摄像头网页 API；`buildToken` 按参数字典序 `k=v&k=v` + secret + t 生成 **MD5 签名 token**（对照迅思维接口文档示例验证一致）；cJSON 解析响应；`probe`/`control`。
...
真实迅思维设备接口的 `direction` 取值与路径（是否带 `/xsw` 前缀）需现场按设备确认；如需调整只需改 `PtzHttpClient::buildControlUrl`。
