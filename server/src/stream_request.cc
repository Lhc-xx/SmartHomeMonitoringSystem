...
{
    streamUrl.clear();
    /* 至少要有 uint16 长度字段；不足时不能把请求降级成 Mock。 */
    if (value.size() < 2U) {
        return false;
    }
    ...
    const std::size_t expectedSize = static_cast<std::size_t>(2U) + length;
    /* 长度必须精确匹配，拒绝截断和长度字段后的垃圾字节。 */
    if (value.size() != expectedSize) {
        return false;
    }

    streamUrl.assign(reinterpret_cast<const char *>(value.data() + 2U), length);
    return true;
}
...
bool parseRecordStartValue(const std::vector<uint8_t> &value, uint64_t &deviceId)
{
    ...
    for (std::size_t index = 0; index < value.size(); ++index) {
        deviceId = (deviceId << 8U) | static_cast<uint64_t>(value[index]);
    }
    return true;
}
